...
#include "CarrierLabPhaseIIID4Commandlet.h"
...
namespace
{
	...
	FString GetOutputRoot(const FString& Params)
	{
		...
		if (!FParse::Value(*Params, TEXT("Out="), OutputRoot))
		{
			...
			OutputRoot = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("CarrierLab"), TEXT("PhaseIII"), TEXT("IIID4"), Stamp);
		}
	...
	FString ResolveReportPath(const FString& Params)
	{
		...
		if (!FParse::Value(*Params, TEXT("Report="), ReportPath))
		{
			ReportPath = FPaths::Combine(
				...
				TEXT("docs"),
				TEXT("checkpoints"),
				TEXT("phase-iii-slice-iiid4-report.md"));
		}
	...
	struct FSlabBreakReplayResult
	{
		...
		FCarrierLabPhaseIIID4SlabBreakPlanAudit PlanAudit;
	...
	FString ComputeSlabBreakReplayHash(const FSlabBreakReplayResult& Result)
	{
		uint64 Hash = 1469598103934665603ull;
		HashMixString(Hash, TEXT("CarrierLab-IIID4-slab-break-plan-replay-v1"));
		...
		HashMixString(Hash, Result.PlanAudit.SourceDestinationMassHash);
		HashMixString(Hash, Result.PlanAudit.SlabBreakPlanHash);
		...
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.DestinationMassRecordCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.AcceptedDestinationMassRecordCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.PlanCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.ValidPlanCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.InvalidPlanCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.WouldDestroySourcePlateCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.TotalRemovedTriangleCount + 1));
		HashMix(Hash, static_cast<uint64>(Result.PlanAudit.TotalSurvivingTriangleCount + 1));
		HashMixDouble(Hash, Result.PlanAudit.TotalRemovedAreaWeight);
		return HashToString(Hash);
	}

	bool RunSlabBreakReplay(
		const int32 Replay,
		...
		FSlabBreakReplayResult& OutResult)
	{
		OutResult = FSlabBreakReplayResult();
		OutResult.Replay = Replay;
		...
		for (int32 Step = 0; Step <= SearchLimit; ++Step)
		{
			...
			if (!Actor->GetPhaseIIIB3SubductionMatrixAudit(OutResult.MatrixAudit) ||
				...
				!Actor->PlanPhaseIIID4SlabBreak(OutResult.PlanAudit, CollisionThresholdKm, 0.5) ||
			...
			const bool bPlanReady =
				OutResult.PlanAudit.PlanCount > 0 &&
				OutResult.PlanAudit.ValidPlanCount == OutResult.PlanAudit.PlanCount &&
				OutResult.PlanAudit.InvalidPlanCount == 0;
			const bool bPureOceanicNegative =
				...
				OutResult.PlanAudit.DestinationMassRecordCount == 0 &&
				OutResult.PlanAudit.PlanCount == 0;
				...
				(bExpectNoPlan && bPureOceanicNegative))
			{
				...
				OutResult.Seconds = FPlatformTime::Seconds() - StartSeconds;
				OutResult.bCompleted = true;
				OutResult.ReplayHash = ComputeSlabBreakReplayHash(OutResult);
				break;
			}
		}
	...
	bool SlabBreakReplayStable(const FSlabBreakReplayResult& A, const FSlabBreakReplayResult& B)
	{
		return A.bCompleted && B.bCompleted &&
			A.Fixture == B.Fixture &&
			...
			A.PlanAudit.SourceDestinationMassHash == B.PlanAudit.SourceDestinationMassHash &&
			A.PlanAudit.SlabBreakPlanHash == B.PlanAudit.SlabBreakPlanHash &&
			A.PlanAudit.PlanCount == B.PlanAudit.PlanCount &&
			A.PlanAudit.ValidPlanCount == B.PlanAudit.ValidPlanCount &&
			A.PlanAudit.InvalidPlanCount == B.PlanAudit.InvalidPlanCount &&
			A.PlanAudit.WouldDestroySourcePlateCount == B.PlanAudit.WouldDestroySourcePlateCount &&
			A.PlanAudit.TotalRemovedTriangleCount == B.PlanAudit.TotalRemovedTriangleCount &&
			A.PlanAudit.TotalSurvivingTriangleCount == B.PlanAudit.TotalSurvivingTriangleCount &&
			...
			FMath::IsNearlyEqual(A.PlanAudit.TotalRemovedAreaWeight, B.PlanAudit.TotalRemovedAreaWeight, 1.0e-9);
	}

	bool SlabBreakPlanPasses(const FSlabBreakReplayResult& A, const FSlabBreakReplayResult& B)
	{
		return SlabBreakReplayStable(A, B) &&
			A.StepCount > 0 &&
			A.StepCount <= MaxThresholdSteps &&
			A.PolarityAudit.CollisionCandidateCount > 0 &&
			A.PlanAudit.InterpenetrationAcceptedGroupCount == 1 &&
			A.PlanAudit.AcceptedDestinationMassRecordCount > 0 &&
			A.PlanAudit.PlanCount > 0 &&
			A.PlanAudit.ValidPlanCount == A.PlanAudit.PlanCount &&
			A.PlanAudit.InvalidPlanCount == 0 &&
			A.PlanAudit.WouldDestroySourcePlateCount == 0 &&
			A.PlanAudit.TotalRemovedTriangleCount > 0 &&
			A.PlanAudit.TotalSurvivingTriangleCount > 0 &&
			A.PlanAudit.TotalRemovedAreaWeight > 0.0 &&
			A.PlanAudit.Plans.Num() == A.PlanAudit.PlanCount &&
			A.PlanAudit.Plans[0].bRemovalSetMatchesTerrane &&
			A.PlanAudit.Plans[0].bTopologyValid &&
			A.PatchTriangleCount > 0 &&
			A.PlanAudit.Plans[0].RemovedTriangleCount >= A.PatchTriangleCount &&
			A.PlanAudit.Plans[0].SurvivingTriangleCount > 0 &&
			A.PlanAudit.Plans[0].NonManifoldEdgeCount == 0 &&
			A.PlanAudit.Plans[0].InvalidMappedTriangleCount == 0 &&
			A.PolicyResolvedMultiHitCount == 0;
	}

	bool PureOceanicNegativePasses(const FSlabBreakReplayResult& A, const FSlabBreakReplayResult& B)
	{
		return SlabBreakReplayStable(A, B) &&
	...
	void AppendSlabBreakJson(const FSlabBreakReplayResult& Result, TArray<FString>& Lines)
	{
		Lines.Add(FString::Printf(
			TEXT("{\"kind\":\"iiid4_slab_break_plan_replay\",\"fixture\":%s,\"replay\":%d,\"completed\":%s,\"read_only_hashes_stable\":%s,\"step\":%d,\"matrix_hits\":%d,\"collision_candidates\":%d,\"accepted_mass_records\":%d,\"plans\":%d,\"valid_plans\":%d,\"invalid_plans\":%d,\"would_destroy_source\":%d,\"removed_triangles\":%d,\"surviving_triangles\":%d,\"patch_seed_triangle\":%d,\"patch_triangle_count\":%d,\"removed_area_weight\":%.9f,\"policy_resolved_multi_hits\":%d,\"destination_mass_hash\":%s,\"slab_break_plan_hash\":%s,\"replay_hash\":%s,\"seconds\":%.6f}"),
			*JsonString(Result.Fixture),
			...
			Result.PlanAudit.AcceptedDestinationMassRecordCount,
			Result.PlanAudit.PlanCount,
			Result.PlanAudit.ValidPlanCount,
			Result.PlanAudit.InvalidPlanCount,
			Result.PlanAudit.WouldDestroySourcePlateCount,
			Result.PlanAudit.TotalRemovedTriangleCount,
			Result.PlanAudit.TotalSurvivingTriangleCount,
			Result.PatchSeedTriangleId,
			Result.PatchTriangleCount,
			Result.PlanAudit.TotalRemovedAreaWeight,
			Result.PolicyResolvedMultiHitCount,
			*JsonString(Result.PlanAudit.SourceDestinationMassHash),
			*JsonString(Result.PlanAudit.SlabBreakPlanHash),
			*JsonString(Result.ReplayHash),
			Result.Seconds));
	}

	FString PlanSummary(const FCarrierLabPhaseIIID4SlabBreakPlanAudit& Audit)
	{
		if (Audit.Plans.IsEmpty())
		{
			return TEXT("none");
		}
		const FCarrierLabPhaseIIID4SlabBreakPlanRecord& Plan = Audit.Plans[0];
		return FString::Printf(
			TEXT("pair `%llu`, source %d -> destination %d, removed %d triangles, surviving %d triangles, cut boundary edges %d, valid %s, plan hash `%s`"),
			static_cast<unsigned long long>(Plan.PairKey),
			Plan.SourcePlateId,
			Plan.DestinationPlateId,
			Plan.RemovedTriangleCount,
			Plan.SurvivingTriangleCount,
	...
	FString BuildReport(
		...
		const FSlabBreakReplayResult& PlanA,
		const FSlabBreakReplayResult& PlanB,
		const FSlabBreakReplayResult& OceanA,
		const FSlabBreakReplayResult& OceanB)
	{
		...
		const bool bPlanPass = SlabBreakPlanPasses(PlanA, PlanB);
		...
		Report += FString::Printf(
			TEXT("| Slab-break dry-run plan deterministic | %s | replay A `%s`, replay B `%s`, plans %d / %d |\n"),
			*PassFail(bPlanPass),
			*PlanA.PlanAudit.SlabBreakPlanHash,
			*PlanB.PlanAudit.SlabBreakPlanHash,
			PlanA.PlanAudit.PlanCount,
			PlanB.PlanAudit.PlanCount);
		Report += FString::Printf(
			TEXT("| Removed set equals terrane and survivor topology valid | %s | removed %d, surviving %d, invalid plans %d, non-destroying source plans %d |\n"),
			*PassFail(bPlanPass),
			PlanA.PlanAudit.TotalRemovedTriangleCount,
			PlanA.PlanAudit.TotalSurvivingTriangleCount,
			PlanA.PlanAudit.InvalidPlanCount,
			PlanA.PlanAudit.WouldDestroySourcePlateCount);
		Report += FString::Printf(
			TEXT("| Pure-oceanic negative emits no detach plans | %s | decisions %d, collision candidates %d, plans %d |\n"),
			*PassFail(bOceanPass),
			OceanA.PolarityAudit.DecisionCount,
			OceanA.PolarityAudit.CollisionCandidateCount,
			OceanA.PlanAudit.PlanCount);
		Report += FString::Printf(
		...
		Report += TEXT("| Fixture | Replay | Step | Matrix hits | Collision candidates | Mass accepted | Plans | Valid | Invalid | Destroy source | Removed tris | Surviving tris | Patch tris | Policy multi-hits | Read-only stable | Hash |\n");
		Report += TEXT("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|\n");
		const FSlabBreakReplayResult* Results[] = { &PlanA, &PlanB, &OceanA, &OceanB };
		for (const FSlabBreakReplayResult* Result : Results)
		{
			Report += FString::Printf(
				...
				*Result->PlanAudit.SlabBreakPlanHash);
		}
		...
		Report += TEXT("The source-patch fixture sculpts an evidence-adjacent seed patch on the source plate after IIID.2 collision evidence exists. The connected continental terrane may expand beyond that seed through normal IIID.1 traversal; the dry-run gate therefore requires the removed terrane to contain the seed patch, preserve surviving source topology, and match the terrane record exactly. Duplicate accepted mass records that reference the same source-terrane removal set are counted and deduped into one plan.\n\n");
		Report += TEXT("All plan calls are audited before and after against projection, state, crust, and convergence hashes. Stable before/after hashes show this slice observes and plans from collision evidence without making topology, motion, or authority changes.\n\n");

		Report += TEXT("## Verdict\n\n");
		Report += bAllPass
			? TEXT("PASS. IIID.4 slab-break detachment planning is deterministic and read-only for the exercised fixtures. IIID.5 may plan suture attachment, but must still avoid topology mutation until the later integrated event slice.\n")
			: TEXT("FAIL. Do not advance to IIID.5 until the failing dry-run slab-break gate is investigated.\n");
		return Report;
	}
}

UCarrierLabPhaseIIID4Commandlet::UCarrierLabPhaseIIID4Commandlet()
{
	IsClient = false;
	IsEditor = true;
	IsServer = false;
...
int32 UCarrierLabPhaseIIID4Commandlet::Main(const FString& Params)
{
	...
	FSlabBreakReplayResult PlanA;
	FSlabBreakReplayResult PlanB;
	FSlabBreakReplayResult OceanA;
	FSlabBreakReplayResult OceanB;
	const bool bPlanA = RunSlabBreakReplay(0, TEXT("source_patch_detach_plan"), true, true, true, true, false, PlanA);
	const bool bPlanB = RunSlabBreakReplay(1, TEXT("source_patch_detach_plan"), true, true, true, true, false, PlanB);
	const bool bOceanA = RunSlabBreakReplay(0, TEXT("pure_oceanic_negative"), false, false, false, false, true, OceanA);
	const bool bOceanB = RunSlabBreakReplay(1, TEXT("pure_oceanic_negative"), false, false, false, false, true, OceanB);
	...
	AppendSlabBreakJson(PlanA, JsonLines);
	AppendSlabBreakJson(PlanB, JsonLines);
	AppendSlabBreakJson(OceanA, JsonLines);
	AppendSlabBreakJson(OceanB, JsonLines);
	...
	const bool bAllPass =
		...
		SlabBreakPlanPasses(PlanA, PlanB) &&
	...
	UE_LOG(LogTemp, Display, TEXT("CarrierLab IIID.4 report: %s"), *ReportPath);
	UE_LOG(LogTemp, Display, TEXT("CarrierLab IIID.4 metrics: %s"), *MetricsPath);
	return bAllPass ? 0 : 1;
}
