...
|---|---|---|
| default_100k_40_manual_step_32 | pass | unresolved `4`, crossDiff bucket `16144`, third bucket `8456`, crossDiff resolved `16141`, third resolved `8455`, distance-tie held `4`, unsupported held `0`, shared `0`, coalesced `0`, co-fire `0`, policy/prior `0/0`, events `0->0`, next `64->64`, mode `phase_iii_e6_live_hold_unresolved_multi_hit_4`, hashes proj `0be461d212b1a54a -> 0be461d212b1a54a`, state `ad21171e53f48d79 -> ad21171e53f48d79`, crust `83de07ebd0edbf2a -> 83de07ebd0edbf2a`, selection `ef0d6cc22e1dfe40` |
| default_100k_40_auto_cadence_step_32 | pass | unresolved `4`, crossDiff bucket `16144`, third bucket `8456`, crossDiff resolved `16141`, third resolved `8455`, distance-tie held `4`, unsupported held `0`, shared `0`, coalesced `0`, co-fire `0`, policy/prior `0/0`, events `0->0`, next `32->32`, mode `phase_iii_e6_live_hold_unresolved_multi_hit_4`, hashes proj `113a31bd102e0eb9 -> 0be461d212b1a54a`, state `db7893beb0c6626a -> ad21171e53f48d79`, crust `1d33b3248c817839 -> 83de07ebd0edbf2a`, selection `ef0d6cc22e1dfe40` |
...
| default_100k_40_manual_step_32_replay | pass | unresolved `4`, crossDiff bucket `16144`, third bucket `8456`, crossDiff resolved `16141`, third resolved `8455`, distance-tie held `4`, unsupported held `0`, shared `0`, coalesced `0`, co-fire `0`, policy/prior `0/0`, events `0->0`, next `64->64`, mode `phase_iii_e6_live_hold_unresolved_multi_hit_4`, hashes proj `0be461d212b1a54a -> 0be461d212b1a54a`, state `ad21171e53f48d79 -> ad21171e53f48d79`, crust `83de07ebd0edbf2a -> 83de07ebd0edbf2a`, selection `ef0d6cc22e1dfe40` |

Same-seed replay: hashes `ef0d6cc22e1dfe40` and `ef0d6cc22e1dfe40`, equal.
Manual vs automatic step-32 equivalence (post-state hashes + held counters + remesh mode): match.
IIIE.6.4 baseline replay reproduces the historical `24600` post-motion holds with `bEnablePhaseIIIE3NearestHitTieBreak = false`; the new schema-additive selection hash for the disabled-path is `a61a52fc310e6597` and the prior schema's `0be461d212b1a54a / ad21171e53f48d79 / 83de07ebd0edbf2a` projection/state/crust still reproduces verbatim because IIIE.6.5's per-record fields are zero on the disabled path.
...
- Prior IIIE.6.4 manual_step_32 selection hash (v2): `0be461d212b1a54a / ad21171e53f48d79 / 83de07ebd0edbf2a` (projection/state/crust). New IIIE.6.4 baseline selection hash (v4, IIIE.6.5 disabled, IIIE.6.6 zero-valued): `a61a52fc310e6597`. The hashes differ from schema/mixer changes; behaviour is identical.
- IIIE.6.5 default 100k/40 seed-42 manual_step_32 selection hash (v4, IIIE.6.5 enabled and IIIE.6.6 disabled): `ef0d6cc22e1dfe40`. This remains the historical nearest-hit blocker-reduction evidence with the residual 4 ties held.
